...
def _list_spaces(client, args, settings) -> list[TextContent]:
    space_ids=settings.genie_space_ids
    LOGGER.info(f"Here is my token : {settings.auth_token}")
    current_user_info = client.current_user.me()
    LOGGER.info(f"Authenticated as User ID: {current_user_info.id}")

    logging.info(f"User Name (Email): {current_user_info.user_name}")
...
class GenieTool(BaseTool):
    ...
    #change
    def execute(self, token: str, **kwargs):
    #def execute(self, **kwargs):
        #change
        return self.func(client=WorkspaceClient(token=token), args=kwargs)
...
def list_genie_tools(settings) -> list[GenieTool]:
    return [
        ...
        GenieTool(
            ...
            description="Lists available Genie spaces, which provide structured data lookup via a text-to-SQL interface. Use this to discover relevant data sources.",
            input_schema=ListSpacesInput.model_json_schema(),
            func=functools.partial(_list_spaces, settings=settings),
        ),
    ]
